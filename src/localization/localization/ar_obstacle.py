...
# Distance constants
_p_bu_m__bu_OBST = {24:[-1.0,0.0,0.0,0.0],12:[-1.0,0.0,0.0,0.0], 9:[-1.7, 0.0, 0.0, 0.0],15:[-1.0,0.0,0.0,0.0]}

_OBST_SEQ = [9]
# yaw const
_YAW_DES = 0.0 # radians

# P(ID) constants, fill these in!!
_P_X = 0.4
_P_Y = 0.2
_P_Z = 0.1
...
class ARObstacleController:
    def __init__(self, hz=60):
        ...
        #self.local_vel_sub = rospy.Subscriber("/mavros/local_position/velocity", TwistStamped, self.local_vel_cb)        
        ...
        self.obstacles = {12 : 3, 24 : 2, 9: 4, 15:0} # dict (marker -> mode)
    ...
    def update_finite_state(self, mode=0, force=False):
        if force:
        ...
        if any(marker.id in self.obstacles for marker in self.markers) and self.finite_state == 0:
            self.switch_pub.publish("ar")
            if _DEBUG: rospy.loginfo("found one!")
            good_markers = [marker for marker in self.markers if marker.id in self.obstacles]
            self.current_obstacle_tag = min(good_markers, key= lambda x: x.pose.pose.position.x).id
            self.finite_state = 1
    ...
    def fly_to_obstacle(self):
        ...
        p_bu_m__bu = [-target_marker.pose.pose.position.x, -target_marker.pose.pose.position.y, -target_marker.pose.pose.position.z]

        curr_yaw = self.get_yaw(target_marker)

        x_error = _p_bu_m__bu_OBST[target_marker.id][0] - p_bu_m__bu[0]
        y_error = _p_bu_m__bu_OBST[target_marker.id][1] - p_bu_m__bu[1]
        z_error = _p_bu_m__bu_OBST[target_marker.id][2] - p_bu_m__bu[2]
        
        curr_yaw *= -np.sign(y_error)
        yaw_error = _p_bu_m__bu_OBST[target_marker.id][3] - curr_yaw
        rospy.loginfo("error: x: %.04f y: %.04f z: %.04f yaw %.04f" % (x_error, y_error, z_error, yaw_error))
        
        if abs(x_error) < 0.1 and abs(y_error) < 0.1 and abs(z_error) < 0.1 and abs(yaw_error) < 0.07: # we can start flying thru
            self.update_finite_state(self.obstacles[target_marker.id])
            self.current_obstacle_seq+= 1 if self.current_obstacle_seq < len(_OBST_SEQ)-1 else 0
            return

        self.vel_hist[0].insert(0,x_error*_P_X)
        self.vel_hist[1].insert(0,y_error*_P_Y)
        self.vel_hist[2].insert(0,z_error*_P_Z)
        self.vel_hist[3].insert(0,curr_yaw*_P_YAW)

    def avoid_ring(self):
        td = datetime.now()-self.t_obstacle_start
        if td.total_seconds() < 2.5:
            self.vel_hist[2].insert(0, 0.3)
            rospy.loginfo("ring avoid: going up!")

        elif td.total_seconds() < 6 and td.total_seconds() > 2.5:
            self.clear_history(z=True)
            self.vel_hist[0].insert(0, 0.3)
    ...
    def avoid_hurdle(self):
        td = datetime.now()-self.t_obstacle_start
        if td.total_seconds() < 0.5:
            self.vel_hist[2].insert(0,0.3)
            rospy.loginfo("hurdle avoid: going up!")
        elif td.total_seconds() < 5 and td.total_seconds() > 0.5:
            self.clear_history(z=True)
            self.vel_hist[0].insert(0,0.3)
            rospy.loginfo("hurdle avoid: going forward!")
        else:
            self.clear_history(x=True, z=True)
            self.t_obstacle_start = None
            self.update_finite_state(force=True)
    ...
    def avoid_gate(self):
        td = datetime.now()-self.t_obstacle_start
        if td.total_seconds() < 1.5:
            self.vel_hist[2].insert(0,-0.3)
            rospy.loginfo("gate avoid: going down!")
        elif td.total_seconds() < 7 and td.total_seconds() > 1.5:
            self.clear_history(z=True)
            self.vel_hist[0].insert(0,0.3)
            rospy.loginfo("gate avoid: going forward")
        else:
            self.clear_history(x=True, z=True)
            self.t_obstacle_start = None
            self.update_finite_state(force=True)
    ...
    def get_yaw(self, marker):
	r_unit_x = [1.0,0.0,0.0]
        p_m_bu__bu = [marker.pose.pose.position.x,marker.pose.pose.position.y,marker.pose.pose.position.z]
	prod = np.dot(r_unit_x,p_m_bu__bu)
	theta = np.arccos(prod/(np.linalg.norm(p_m_bu__bu)))
	return theta
    
    def clear_history(self,x=False, y=False, z=False, yaw=False, wipe=False):
        if wipe:
            self.vel_hist = [[],[],[],[]]
            return
        if x:
